...
const Supplier = () => {
  ...
  const filteredSuppliers = suppliers.filter((supplier) => {
    return supplier.name.toLowerCase().includes(searchKeyword.toLowerCase());
  });

  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await supplierApi.getSupplier();
  ...
  const handleHide = async (supplierId) => {
    const confirmHide = window.confirm(
      "Are you sure you want to hide this Supplier?"
    );
    if (confirmHide) {
      try {
        await supplierApi.removeSupplier(supplierId);
        const updatedSupplier = suppliers.filter(
          (supplier) => supplier._id !== supplierId
        );
        setSuppliers(updatedSupplier);
        toast.success("Hide Supplier Successfully!!!");
      } catch (error) {
          ...
          <tbody>
            {filteredSuppliers.map((supplier) => (
              ...
              >
                ...
                <td className="px-6 py-4">{supplier.type}</td>
                <th
                  scope="row"
                  className="px-6 py-4 font-medium text-gray-900 whitespace-nowrap dark:text-white"
                >
                ...
                <td className="px-6 py-4">
                  <input type="checkbox" checked={supplier.isHidden} />
                </td>
                {/* <td className="px-6 py-4">{supplier.status}</td> */}
                <td className="px-2 py-2">
                  <Link to={`/admin/edit-supplier/${supplier._id}`}>
                    <button className="font-medium text-blue-600 dark:text-blue-500">
                      Edit
                    </button>
                  </Link>
                  <button
                    onClick={() => handleHide(supplier._id)}
